...
def init():
  ...
  #==========================================================================================================functionality for button======================================================
    def click():
        ...
        cursor = db.cursor()
        cursor.execute('SELECT pass FROM for_signup WHERE pass = %s', (current_password,))
        result = cursor.fetchone()

        if result is not None:
            cursor.execute('UPDATE for_signup SET pass = %s WHERE pass = %s', (new_password, current_password))
            db.commit()
            entry1.delete(0, END)
            entry2.delete(0, END)
            entry3.delete(0, END)
